...
async def callbacks(call: types.CallbackQuery):
    ...
    # Выбор города
        ...
        await bot.edit_message_text(f"Город выбран: {city}\nВыберите категорию:", user_id, call.message.message_id, reply_markup=category_menu("place"))
    ...
    # Выбор категории
        ...
        user_states[user_id]["step"] = "choose_subcategory"
        await bot.edit_message_text(f"Категория выбрана: {category}\nВыберите подкатегорию:", user_id, call.message.message_id, reply_markup=subcategory_menu(category, "place"))

    elif data.startswith("find_") and data not in ["find_service"]:
        category = data.replace("find_", "")
        user_states[user_id]["data"]["category"] = category
